...
def isolate(s,dist1_num,dist2_num):
	...
	HUMAN_DEFENCE_POWER = HUMAN_DEFENCE_POWER + 0.2
	new_state = copy_state(s)
	zombie_action(new_state) 
	infected_zombify(new_state)
	return new_state
...
def can_isolate(s,dist1_num,dist2_num):
	...
	human, infected, zombie = s[dist_str(dist1_num)]
	if human <= 0.7*(human+infected+zombie): return False
	return True

def develop_weapon(s,dist1_num,dist2_num):
	global HUMAN_ATTACK_POWER
	HUMAN_ATTACK_POWER = HUMAN_ATTACK_POWER + 1
	new_state = copy_state(s)
	zombie_action(new_state) 
	infected_zombify(new_state)
	return new_state
...
def can_develop_weapon(s,dist1_num,dist2_num):
	...
	if dominated_by(s,dist1_num) != "zombie": return False
	human, infected, zombie = s[dist_str(dist1_num)]
	if human <= 0.6*(human+infected+zombie): return False
	return True
