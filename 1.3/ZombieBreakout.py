...
def DESCRIBE_STATE(state):
	...
	print("+", end="")
	print(("{:-^42}".format('') + "+" )*LENGTH)
	for row in output_grid:
		print("|",end="")
		for col in row:
			print("{:^42}".format(col), end="|")	
...
def develop_weapon(s,dist1_num):
	pass

def can_develop_weapon(s,dist1_num):
	return -1


def develop_medicine(s,dist1_num):
	pass

def can_develop_medicine(s,dist1_num):
	return -1
...
def dist_operator_fomulator(s):
	...
	for dist in range(DIST_SIZE):
		...
		if best_action != "nothing":
			new_state = action_function[action_name.index(best_action)](new_state,dist)
